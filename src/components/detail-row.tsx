export default function DetailRow({
  ...
  return (
    <div className="xs:flex-row xs:flex-wrap xs:items-center xs:justify-between xs:gap-2 flex flex-col gap-1.5 py-1.5">
      <span className="text-muted-foreground shrink-0 text-sm">{label}</span>
      <span className={`min-w-0 break-all ${variantClasses[variant]}`}>{value}</span>
    </div>
  );
}
